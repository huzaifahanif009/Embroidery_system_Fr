...
@Component({selector:'erp-vendors',standalone:true,
...
<erp-page-header title="Vendors" (newClick)="openCreate()"></erp-page-header>
...
<erp-modal [(visible)]="showModal" [header]="modalTitle" [mode]="mode" [saving]="saving" (save)="onSave()" (cancel)="close()" width="580px">
  <form [formGroup]="form" class="fg">
    <div class="tw-form-group"><label class="tw-label-field">Code *</label><input formControlName="code" class="tw-input" [readonly]="mode==='view'" /></div>
        ...
        <div class="tw-form-group"><label class="tw-label-field">Currency</label><select formControlName="currency" class="tw-select" [attr.disabled]="mode==='view'?true:null"><option value="PKR">PKR</option><option value="USD">USD</option><option value="EUR">EUR</option></select></div>
...
export class VendorsComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit():void{
    this.rows=[...(this.mock.vendors as unknown[])];
    this.form=this.fb.group({code:['',Validators.required],name:['',Validators.required],contactPerson:[''],phone:[''],email:[''],paymentTerms:[30,Validators.required],currency:['PKR'],status:['active']});
  }
  ...
  onAction(e:{action:string;data:unknown}):void{
    ...
    if(e.action==='delete'){
      this.confirm.confirm({
        ...
        accept:()=>{
          (this.mock.vendors as Record<string,unknown>[])=(this.mock.vendors as Record<string,unknown>[]).filter(x=>x['id']!==row['id']);
          this.rows=[...(this.mock.vendors as unknown[])];
  ...
  onSave():void{
    ...
    setTimeout(()=>{
      ...
      if(this.mode==='create') (this.mock.vendors as Record<string,unknown>[]).push({...val,id:Date.now().toString()});
      else if(this.selected){
        const i=(this.mock.vendors as Record<string,unknown>[]).findIndex(x=>x['id']===this.selected!['id']);
        if(i>-1) (this.mock.vendors as Record<string,unknown>[])[i]={...(this.mock.vendors as Record<string,unknown>[])[i],...val};
      }
      this.rows=[...(this.mock.vendors as unknown[])];
